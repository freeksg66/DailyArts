package com.github.dailyarts.ui.widget.imagespickers.container;
...
import com.github.dailyarts.ui.widget.imagespickers.preview.MultiImgShowActivity;
...
public class SimpleImageAdapter extends CommonAdapter<String> {
	...
	private void initImgSize() {
		...
		if (isDelete) {
            ...
            int size = containerWidth - Utils.dp2px(mContext, 3 * (rowCount -1) + 8 * rowCount);
            size = Math.round(size / (float)rowCount);
            fl = new FrameLayout.LayoutParams(size, size);
            //填充上右，为删除按钮让出空间
			fl.setMargins(0, Utils.dp2px(mContext, 8), Utils.dp2px(mContext, 8), 0);
		}
		//不带删除
		else {
			// 如果行数为4，中间间隔3个10dp共30dp
			int size = containerWidth - Utils.dp2px(mContext, 10 * (rowCount -1));
			size = Math.round(size / (float)rowCount);
			fl = new FrameLayout.LayoutParams(size, size);
		}
	}
	...
	public SimpleImageAdapter(ViewGroup container, boolean isDelete, int rowCount) {
		super(container.getContext(), null, R.layout.image_selector_item_view);
